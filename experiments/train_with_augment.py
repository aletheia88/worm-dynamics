...
def train(
    ...
    for n_epoch in tqdm(range(num_epochs)):
        ...
        training_loss = []
        for n_iter, (inputs, _, _, _, _) in \
                tqdm(enumerate(training_dataloader)):

            if n_iter == num_iterations:
                break

            targets = inputs[:, target_indices, :]
            num_samples, _, window_size = inputs.shape

            # choose a random half of all neurons to augment
            augment_indices = np.random.choice(
                    neuron_indices, num_neurons // 2)

            # sample noise from gaussian distribution centered at 0
            noise = torch.normal(
                    mean=0, std=sigma,
                    size=(num_samples, num_neurons // 2, window_size),
                    device=device)
            augmented_inputs = inputs[:, input_indices, :]
            augmented_inputs[:, augment_indices, :] = \
                augmented_inputs[:, augment_indices, :] + noise

            # model predicts activity
...
if __name__ == '__main__':
    ...
    experiment = f"depth{depth}_nfmaps{num_fmaps}_lr{lr}_{attention_scheme}_norm0505_aug{sigma}"
