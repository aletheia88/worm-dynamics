...
def assemble_all(neuron_classes, behavior_index_dict, max_len=1600, normalize=True):
    datasets = []
    ...
    assembled_data = np.zeros(
        (len(unique_datasets), num_neurons + num_behaviors, max_len)
    )
    ...
    if normalize:
        ...
        for i in range(num_neurons):
            ...
            assembled_data[:, i, :] = (
                all_neural_traces / (2 * np.mean(all_neural_traces)) - 1
            )
        ...
        normalized_head_angle = (
            2 * ((all_head_angle_traces - min_val) / (max_val - min_val)) - 1
        )
...
def load_behaviors(max_len: int):
    ...
    for prj, data in prj_data.items():
        ...
        ds_of_interest = [
            ds
            for ds, value in structure_data.items()
            if len(
                set(["neuropal"] if prj == "kfc" else ["wt"]).intersection(
                    set(value["Tags"])
                )
            )
            > 0
        ...
        for ds in ds_of_interest:
            ...
            if (
                len(behaviors["velocity"]) >= max_len
                and len(behaviors["head_angle"]) >= max_len
            ):
...
def assemble(neuron_classes: List[str], max_len: int, max_animals: int):
...
def filter_datasets(neuron_classes: List[str]):
    ...
    for prj, data in prj_data.items():
        ...
        for neuron_class in neuron_classes:
            neuron_list += [
                neuron
                for neuron in label_data.keys()
                if neuron.startswith(neuron_class)
            ]

        # find all datasets containing each neuron
        neuron_datasets = {
            neuron: [ds for ds in label_data[neuron].keys()] for neuron in neuron_list
        }
        ...
        for dataset, neurons in dataset_neurons.items():
            for neuron in neurons:
                for neuron_class in neuron_classes:
                    if (
                        neuron.startswith(neuron_class)
                        and neuron_class not in contained_neuron_classes[dataset]
                    ):
        ...
        filtered_datasets = [
            ds
            for ds, classes in contained_neuron_classes.items()
...
def filter_dataset_paths(neuron_classes: List[str]):
    ...
    contained_neuron_classes = {
        ds_path.split("/")[-1].split(".")[0]: [] for ds_path in dataset_paths
    }
    ...
    for dataset_path in dataset_paths:
        ...
        for n_id, info_dict in data["labeled"].items():
            for neuron_class in neuron_classes:
                ...
                if (
                    neuron.startswith(neuron_class)
                    and neuron not in contained_neuron_classes
                ):
...
def load_single_neuron_class(
    ...
    for prj, data in prj_data[source].items():
        ...
        neurons = [
            ...
            for neuron in label_data.keys()
            if neuron.startswith(neuron_class)
            and (len(neuron) - len(neuron_class)) <= 1
            and not neuron in set(neuron_classes) - {neuron_class}
            and (("?" != neuron[-2]) if filter_qs else True)
        ]
        for neuron in neurons:
            count = 0
            ...
            for ds, items in label_data[neuron].items():
                ...
                if ds in ds_of_interest and items["confidence"] > confidence_threshold:
                    file_path = f"{data['processed_h5']}/{ds}-data.h5"
                    loaded_data = h5_to_dict(file_path)
                    gcamp_data = loaded_data["gcamp"]["trace_array_original"]
                    behaviors = loaded_data["behavior"]
                    ...
                    std_behaviors.append(
                        ...
                        np.array([behaviors["velocity"][:max_len]]).T
                    )
...
def assemble_data_with_missing_neurons(neuron_classes, max_len):
    ...
    max_datasets = max(
        [len(all_data[neuron_class]["datasets"]) for neuron_class in neuron_classes]
    )
        ...
        for neuron_index, neuron_class in enumerate(neuron_classes):
            ...
            if neuron_class != max_neuron_class:
                if dataset in neuron_class_datasets:
                    ...
                    assembled_gcamp_traces[dataset_index, :, neuron_index] = all_data[
                        neuron_class
                    ]["gcamp_traces"][gcamp_index, :]
                ...
                assembled_gcamp_traces[dataset_index, :, neuron_index] = all_data[
                    max_neuron_class
                ]["gcamp_traces"][dataset_index, :]

    return (
        assembled_gcamp_traces,
        assembled_behaviors,
        assembled_datasets,
        dataset_neuron_info,
    )
...
if __name__ == "__main__":
    ...
    neuron_classes = ["AVA", "MC", "SMDV", "RID", "AVE", "RIV", "AVD", "AIN"]
    num_neurons = len(neuron_classes)
    behavior_index_dict = {
