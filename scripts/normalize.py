            ...
            for i in range(num_inputs):
                ...
                # ignore the heat-stim column indexed at 'num_neurons - 1'
                    ...
                    for ds_index in range(len(datasets)):
                        ...
                        if (
                            np.min(data[ds_index, i, :])
                            == np.max(data[ds_index, i, :])
                            == 0
                        ):
                            data[ds_index, i, :] = -10
                        else:
    ...
    for i in range(num_inputs):
        ...
        if i == num_neurons - 1:
            ...
            for ds_index in range(num_datasets):
                if (
                    np.max(variable_traces[ds_index, :]) != 0
                    and np.min(variable_traces[ds_index, :]) != 0
                ):
...
def get_linear_tfm_params(ds_name, upper_percentile=100, lower_percentile=0):
    ...
    for i in range(num_inputs):
        ...
        upper_bound, lower_bound = np.percentile(
            traces, [upper_percentile, lower_percentile]
        )
