...
class Head(torch.nn.Module):
    def __init__(self, attention_mask, embedding_dims, attention_dims, device):
        ...
        self.query_weights = torch.nn.ModuleList(
            [
                torch.nn.Linear(*attention_dims["w_q"], bias=False),
                torch.nn.Linear(
                    *attention_dims["q"],
                    bias=False,
                ),
        ...
        self.value_weights = torch.nn.Linear(
            embedding_dims, embedding_dims, bias=False, device=device
        )
    ...
    def forward(self, key, query, value):
        ...
        attention_matrix = (weighted_query @ weighted_key).transpose(
            1, 2
        ) / self.sqrt_dk
        # apply attention masking
        attention_matrix = attention_matrix.masked_fill(
            self.attention_mask, float("-inf")
        )
...
class MultiHeadAttention(torch.nn.Module):
    def __init__(
        ...
        self.head = Head(attention_mask, embedding_dims, attention_dims, device)
...
class AttentionBlockMini(torch.nn.Module):
    def __init__(self, embedding_dims, N, B, attention_scheme, device):
        ...
        self.attention = MultiHeadAttention(
            attention_mask, embedding_dims, attention_dims, num_heads=1, device=device
        )
